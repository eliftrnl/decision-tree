...
public class TableColumnsController : ControllerBase
{
        ...
        CancellationToken ct)
    {
        ...
        var table = await _db.DecisionTreeTables
            .FirstOrDefaultAsync(x => x.Id == tableId && x.DecisionTreeId == dtId, ct);
        ...
        var column = new TableColumn
        {
            ...
            IsUniqueIdentifier = request.IsUniqueIdentifier,
        ...
        // Increment DecisionTree SchemaVersion when column is added
        var decisionTree = await _db.DecisionTrees
            .FirstOrDefaultAsync(x => x.Id == dtId, ct);
        if (decisionTree != null)
        {
            decisionTree.SchemaVersion++;
            decisionTree.UpdatedAtUtc = DateTime.UtcNow;
        }
        ...
        CancellationToken ct)
    {
        ...
        // Track if schema-affecting changes were made
        ...
        column.DataType = (ColumnDataType)request.DataType;
        column.IsRequired = request.IsRequired;
        column.IsUniqueIdentifier = request.IsUniqueIdentifier;
        ...
        // Increment SchemaVersion if schema-affecting changes detected
        if (schemaChanged)
        {
            var decisionTree = await _db.DecisionTrees
                .FirstOrDefaultAsync(x => x.Id == dtId, ct);
        ...
        CancellationToken ct)
    {
        ...
        // Increment DecisionTree SchemaVersion when column is deleted
        var decisionTree = await _db.DecisionTrees
            .FirstOrDefaultAsync(x => x.Id == dtId, ct);
        if (decisionTree != null)
        {
        ...
        CancellationToken ct)
    {
        ...
        // Increment SchemaVersion when columns are reordered
        var decisionTree = await _db.DecisionTrees
            .FirstOrDefaultAsync(x => x.Id == dtId, ct);
        if (decisionTree != null)
        {
        ...
        CancellationToken ct = default)
    {
        ...
        return Ok(new
        {
            ...
            newSchemaVersion = decisionTree?.SchemaVersion
        });
    }
}
