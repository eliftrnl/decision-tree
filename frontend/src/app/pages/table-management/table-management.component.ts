...
export class TableManagementComponent implements OnInit {
  ...
  private createTable(): void {
    ...
    const payload = {
      ...
      direction: Number(this.form.direction),
      statusCode: Number(this.form.statusCode ?? 1)
    };

    this.tableService.createTable(this.decisionTreeId, payload).subscribe({
      next: () => {
        this.closeModal();
        this.loadTables();
        this.loading.set(false);
      ...
      error: (err: any) => {
        ...
        this.error.set(apiMessage || 'Tablo eklenirken bir hata oluştu.');
        console.error(err);
        this.loading.set(false);
      }
    });
  ...
  private updateTable(): void {
    ...
    this.tableService.updateTable(this.decisionTreeId, table.id, payload).subscribe({
      next: () => {
        this.closeModal();
        this.loadTables();
        this.loading.set(false);
      ...
      error: (err: any) => {
        const apiMessage = err?.error?.message;
        this.error.set(apiMessage || 'Tablo güncellenirken bir hata oluştu.');
        console.error(err);
        this.loading.set(false);
      }
    });
